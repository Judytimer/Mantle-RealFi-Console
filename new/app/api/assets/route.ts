...
export const GET = async (request: Request) => {
  ...
  if (!hasSupabaseConfig) {
    const fallbackAssets = mockAssets.map((asset) => ({
      ...asset,
      type:
        asset.type === 'bonds'
          ? 'fixed-income'
    ...
    return NextResponse.json({
      ...
      page: 1,
      pageSize: 20,
      warning: 'Supabase not configured. Returning fallback assets.',
    })
  }
  ...
  if (error) {
    ...
    return NextResponse.json({
      ...
      page: 1,
      pageSize: 20,
      warning: 'Supabase query failed. Returning fallback assets.',
    })
  }
