...
export const GET = async (
  ...
  if (!hasSupabaseConfig) {
    const fallback = mockAssets.find((asset) => asset.id === id)
    if (!fallback) {
    ...
    const normalizedType =
      fallback.type === 'bonds'
        ? 'fixed-income'
        : fallback.type === 'invoices'
          ? 'private-credit'
          : fallback.type === 'cash-flow'
            ? 'alternatives'
            : fallback.type
    ...
    return NextResponse.json({
      asset: {
        ...
        confidenceFactors: [],
        realWorld: null,
        yieldHistory: fallback.yieldHistory ?? [],
        navHistory: fallback.navHistory ?? [],
      },
      userPosition: { amount: 0, shares: 0 },
      warning: 'Supabase not configured. Returning fallback asset.',
    })
  }
  ...
  if (assetError || !asset) {
    ...
    if (fallback) {
      ...
      return NextResponse.json({
        ...
        userPosition: { amount: 0, shares: 0 },
        warning: 'Supabase query failed. Returning fallback asset.',
      })
    }
